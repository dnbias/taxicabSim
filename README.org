...
- Al termine della simulazione stampa i dati finali richiesti dal progetto.
  ...
  - I processi taxi che hanno:
    ...
    + Viaggiato più a lungo nel servire una richiesta
...
- Avvia il ciclo d'esecuzione, il quale sceglierà casualmente le celle che diventeranno i punti d'arrivo dei clienti durante la simulazione.

*** Funzioni
#+BEGIN_SRC C
void logmsg(char *message, enum Level l)
#+END_SRC
- Si occupa delle stampe a video.
#+BEGIN_SRC C
Void handler(int sig)
#+END_SRC
- Si occupa della gestione dei segnali.
...
- si sposta la sorgente piu' vicina, ovvero al punto d'origine del cliente
...
- Il processo continuerà a ripetersi fino al termine del tempo a disposizione alla simulazione, o nel caso il Taxi raggiunga il tempo di SO_TIMEOUT tra uno spostamento e l'altro (nel caso non riesca a prendere in carico una richiesta in tempo)
...
** Configurazione
Il programma utilizza dieci parametri letti a tempo di esecuzione, da file taxicab.con, e due a tempo di compilazione, da general.h.3
...
- SO_CAP_MIN
  - Capacità minima di ogni cella.
- SO_CAP_MAX
  - Capacità massima di ogni cella.
- SO_TIMENSEC_MIN
...
- SO_TIMEOUT
  - Tempo di inattività massiva di ogni taxi, dopo il quale il taxi chiude inviando SIGUSR1 a generator.
- SO_DURATION
